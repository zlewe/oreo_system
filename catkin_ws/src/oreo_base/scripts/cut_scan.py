...
def callback(data):
   #pass
	...
	for i in range(int(laser_angular_range/angle_increment_degree)):
		x = middle_index-int(laser_angular_range/angle_increment_degree/2)+i
		if data.ranges[x] < 15:
			if i == 0:
				laser.ranges.append(data.ranges[x+1])
			else:
				laser.ranges.append(data.ranges[x-1])
		else:
...
rospy.Subscriber("/sick_scan", LaserScan, callback)

pub = rospy.Publisher('RosAria/cmd_vel', Twist, queue_size=10)
laser_pub = rospy.Publisher('/sick_scan2', LaserScan, queue_size=1000)
rate = rospy.Rate(40)
